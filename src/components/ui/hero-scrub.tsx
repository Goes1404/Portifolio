...
export type HeroScrubProps = {
  ...
  /** The scrollable wrapper div — needed to set up ScrollTrigger.scrollerProxy */
...
export function HeroScrub({
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.from(titleTopRef.current, { opacity: 0, y: 24, duration: 0.9, ease: "expo.out" }, 0.45);
      tl.from(titleBottomRef.current, { opacity: 0, y: -24, duration: 0.9, ease: "expo.out" }, 0.55);
    }, sectionRef);
    return () => ctx.revert();
  }, [reduced]);

  // ── Scroll-scrub + ScrollerProxy ───────────────────────────────────────────
  useEffect(() => {
    if (reduced || !ready || !framesOk) return;
    const section = sectionRef.current;
    if (!section) return;

    // Determine the scroller — custom div or window
    const scroller: HTMLElement | Window = scrollerEl ?? window;

    // Register scrollerProxy so GSAP can measure a div-based scroller
    if (scrollerEl) {
      ScrollTrigger.scrollerProxy(scrollerEl, {
        ...
        // "fixed" is best for overlays that use position:sticky internally
        pinType: "fixed",
      });
    }
    ...
    const ctx = gsap.context(() => {
      ...
      master.to(titleBottomRef.current, { x: 0, opacity: 1, letterSpacing: "-0.04em", ease: "power2.inOut", duration: 0.22 }, 0.78);
    ...
    return () => {
      ctx.revert();
      // Clean up scrollerProxy when component unmounts
      if (scrollerEl) {
        ScrollTrigger.scrollerProxy(scrollerEl, undefined as any);
        ScrollTrigger.refresh();
      }
    };
  }, [ready, framesOk, reduced, aspect, frameCount, scrollerEl]);
  ...
  return (
    ...
    >
      {/* The sticky child — stays fixed in viewport while section scrolls past */}
      <div className="sticky top-0 flex h-screen w-full flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 flex h-full w-full flex-col items-center justify-center gap-3 md:gap-4">
          ...
          <div
            ...
            style={{
              width: `min(96vw, calc(72vh * ${aspect}))`,
              height: `min(72vh, 96vw / ${aspect})`,
              aspectRatio: aspect,
            }}
          >
            ...
            {framesOk ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 gap-3">
                <span className="text-white/40 text-sm">Carregando frames...</span>
              </div>
            )}
          </div>
