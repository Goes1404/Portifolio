...
export const useShaderBackground = () => {
  ...
  const visibleRef = useRef(true); // false while the canvas is scrolled off-screen
...
void main(){gl_Position=position;}`;
    ...
    render(now = 0) {
      ...
      gl.uniform1f((program as any).time, now * 1e-3 * 0.45);
  ...
  const loop = (now: number) => {
    ...
    // Skip GPU work while the hero is scrolled out of view or the tab is hidden;
    // keep the rAF alive so it resumes instantly when it returns on-screen.
  ...
  useEffect(() => {
    ...
    const canvas = canvasRef.current;
    // Bail gracefully if WebGL2 is unavailable — leaves the static black canvas
    // instead of throwing (which would blank the whole page).
    ...
    // Reduced motion: draw a single calm frame instead of animating.
    if (reduced) {
...
// Clean background layer component
...
void main(void) {
	...
	for (float i=1.; i<12.; i++) {
		...
		col=mix(col,vec3(bg*.02,bg*.15,bg*.38),d);
	}
	O=vec4(col,1);
}`;
